...
### 5. Dynamic Bedtime Suggestions with Caffeine Recalculation — ✅ Resolved
...
* **Root Cause & Technical Investigation**:
  * The main bedtime variable updated reactively, but recommendation strings (e.g. *"Visez le coucher avant 22:30 — votre fenêtre circadienne s'ouvre alors."*) were pre-computed static strings generated by the backend recommendation engine in `/session/{sessionId}/recommendations`.
* **Backend Resolution**:
  * The backend recommendation generator now dynamically substitutes the caffeine-shifted bedtime (`caffeineShiftedBedtime`) into sleep recommendation strings and broadcasts updated data via WebSocket.
* **Frontend Verification**:
  * **[recomendations_controller.dart](file:///Users/saharaislam/Desktop/Tahmid/chrisimhof/lib/features/recomendations/controller/recomendations_controller.dart)**: Listens for socket triggers and refetches updated recommendations smoothly (`refetchRecommendations(silent: true)`).
  ...
  * **[dashboard_controller.dart](file:///Users/saharaislam/Desktop/Tahmid/chrisimhof/lib/features/dashboard/main_dashboard/controller/dashboard_controller.dart)**: Re-evaluates `tonightBedtime` and forwards dynamic bedtime strings to `SleepController` and `for_you_section.dart`.

---

### 6. Dynamic Meal Timing & Fasting-Aware Recommendations — ✅ Resolved
...
* **Backend Resolution**:
  * The backend recommendation engine (`/session/{sessionId}/recommendations`) now applies adaptive nutrition decision rules:
    * **Long Fast (> 5–6 hours) + High Remaining Meal Deficit**: Recommends a substantial/balanced meal (complex carbs + protein) rather than a light snack.
    * **Close to Bedtime (< 2–3 hours)**: Prioritizes sleep protection and recommends a light meal to prevent sleep disruption, regardless of meal deficit.
    * **Post-Workout**: Recommends a protein/recovery-focused meal.
* **Frontend Verification**:
  * `RecommendationController` receives updated nutrition advice in `grouped.Nutrition` and `forYouPreview`.
  * `NutritionController` dynamically reflects meal progress, logged vs planned counts, and `sleepImpactNote` from live scores.
...
### 8. Sleep Quality Score 0 & Scoring Formula Explanation — ✅ Resolved
...
* **Backend Resolution**:
  * The backend now enforces a baseline score floor (minimum score clamp of 10–20 instead of 0) and graceful outlier handling so users recording long or fragmented recovery sleeps no longer receive an absolute 0 score.
  * Backend returns explanatory context where applicable via `tonightNote`.
* **Frontend Verification**:
  * `SleepController.updateFromLiveScoresTab()` dynamically parses `quality: (item['quality'] as num?)?.toInt() ?? 85`.
  * `sleep_history_list.dart` renders the score with localized label (`'${'quality'.tr} ${log.quality}'`).
  * `tonight_bedtime_card.dart` displays `tonightNote` context if provided.
...
### 11. Work Shift-Aware Bedtime Recommendation Calculation — ✅ Resolved
...
* **Backend Resolution**:
  * The backend calculation engine now queries tomorrow's scheduled shift start time and backward-calculates `optimalBedtime`:
    1. **Target Wake-Up Time**:
       $$\text{Target Wake-Up Time} = \text{Tomorrow's Shift Start Time} - \text{Pre-Work Buffer (~2h)}$$
  ...
  * The resulting backward-calculated time is injected into `optimalBedtime` (both time string and hours until bed), `sleepTabData['tonightBedtime']`, and sleep recommendation strings (`body`).
* **Frontend Verification**:
  * **[dashboard_controller.dart](file:///Users/saharaislam/Desktop/Tahmid/chrisimhof/lib/features/dashboard/main_dashboard/controller/dashboard_controller.dart)**: Reactively parses `optimalBedtime` (whether returned as Map or String), updating `dashboardData.optimalBedtime`.
  * **[bedtime_card.dart](file:///Users/saharaislam/Desktop/Tahmid/chrisimhof/lib/features/dashboard/main_dashboard/widgets/bedtime_card.dart)** & **[sleep_orbit_widget.dart](file:///Users/saharaislam/Desktop/Tahmid/chrisimhof/lib/features/dashboard/main_dashboard/widgets/sleep_orbit_widget.dart)**: Dynamically render the shift-aware optimal bedtime and update the circadian beacon position.
  * **[tonight_bedtime_card.dart](file:///Users/saharaislam/Desktop/Tahmid/chrisimhof/lib/features/dashboard/sleep/widgets/tonight_bedtime_card.dart)**: Dynamically displays the shift-aware target sleep window (`bedtime` and `wakeup`).
  * **[sleep_controller.dart](file:///Users/saharaislam/Desktop/Tahmid/chrisimhof/lib/features/dashboard/sleep/controller/sleep_controller.dart)**: Automatically initializes bedtime wheel/picker to the shift-aware recommended time.

---

*Document Updated: 2026-09-07*
